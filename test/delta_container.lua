  ...
  describe("delta_listener", function()
    ...
    it("should listen to 'add' operation", function()
        ...
        container:listen("players/:string", function (change)
            assert.are.same(change.operation, "add")
            assert.are.same(change.raw_path, {"players", "three"})
            assert.are.same(change.path.string, "three")
            assert.are.same(change.value, 3)
        end)
    ...
    it("should match the full path", function()
        container:listen(":name/x", function(change)
            assert.are.same(change.path.name, "entity")
            assert.are.same(change.value, 50)
        end);

        container:listen(":name/xp", function (change)
            assert.are.same(change.raw_path, {"entity", "xp"})
            assert.are.same(change.path.name, "entity")
            assert.are.same(change.value, 200)
        end);
    ...
    it("should listen to 'remove' operation", function()
        container:listen("players/:name", function (change)
            assert.are.same(change.raw_path, { "players", "two" })
            assert.are.same(change.path.name, "two");
            assert.are.same(change.value, nil);
        end)
    ...
    it("should fill multiple variables on listen", function()
        container:listen("entities/:id/:attribute", function(change)
            if (change.path.id == "one") then
                assert.are.same(change.path.attribute, "x")
                assert.are.same(change.value, 20)

            elseif (change.path.id == "two") then
                assert.are.same(change.path.attribute, "y")
                assert.are.same(change.value, 40)
            end
        end)
    ...
    it("should create custom placeholder ", function()
        ...
        container:listen("entity/:xyz", function(change)
            if (change.path.xyz == "x") then assert.are.same(change.value, 1)
            elseif (change.path.xyz == "y") then assert.are.same(change.value, 2)
            elseif (change.path.xyz == "z") then assert.are.same(change.value, 3)
    ...
    it("should remove specific listener", function()
        container:listen("players/ten", function(change)
          assert.are.same(change.value.ten, 10)
        end)
    ...
    it("using the same placeholder multiple times in the path", function()
        container:listen("chests/:id/items/:id", function(change)
            ...
            --
            assert.are.same(change.path.id, "two")
            assert.are.same(change.value, 2)
        end)
    ...
    it("should trigger default listener as fallback", function()
        ...
        container:listen("players/:string", function (change)
            ...
            elseif (change.operation == "remove") then
                assert.are.same(change.path.string, "two");
                assert.are.same(change.value, nil);
            end
        ...
        container:listen(function (change)
            assert.are.same(change.path, { "entity", "rotation" })
            assert.are.same(change.operation, "replace")
            assert.are.same(change.value, 90)
            numCallbacks = numCallbacks + 1
        end)
        ...
        container:set(data)
        assert.are.same(numCallbacks, numCallbacksExpected)
    end)
