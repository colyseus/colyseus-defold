...
return function()
  ...
  local ArraySchemaInsertOps = require 'test.schema.ArraySchemaInsertOps.ArraySchemaInsertOps'
  local ResyncState = require 'test.schema.ResyncFixtures.ResyncState'
  local ResyncArrayState = require 'test.schema.ResyncFixtures.ResyncArrayState'
  local ResyncStateV1 = require 'test.schema.ResyncFixtures.ResyncStateV1'
  local ResyncTransientState = require 'test.schema.ResyncFixtures.ResyncTransientState'

  local function count_refs(decoder)
    local n = 0
    for _ in pairs(decoder.refs.refs) do n = n + 1 end
    return n
  end
  ...
  describe("schema serializer", function()
    ...
    -- Byte tables are raw wire values — the decoder applies its own +1 shift.
    --

    it("ArraySchemaUnshift", function()
      -- ADD at an occupied index = insert (not only index 1)
      local snapshot = { 128, 1, 129, 2, 130, 3, 255, 1, 128, 0, 1, 128, 1, 2, 128, 2, 3 }

      -- consecutive unshift: [1,2,3] -> unshift(0); unshift(-1)
      local state = ArraySchemaInsertOps:new()
      local decoder = Decoder:new(state)
      decoder:decode(snapshot)
      decoder:decode({ 255, 1, 128, 0, 255, 128, 1, 0 })
      assert_equal(5, state.numbers:length())
      assert_equal(-1, state.numbers.items[1])
      assert_equal(0, state.numbers.items[2])
      assert_equal(1, state.numbers.items[3])
      assert_equal(2, state.numbers.items[4])
      assert_equal(3, state.numbers.items[5])

      -- multi-item unshift: [1,2,3] -> unshift(-1, -2)
      state = ArraySchemaInsertOps:new()
      decoder = Decoder:new(state)
      decoder:decode(snapshot)
      decoder:decode({ 255, 1, 128, 0, 255, 128, 1, 254 })
      assert_equal(5, state.numbers:length())
      assert_equal(-1, state.numbers.items[1])
      assert_equal(-2, state.numbers.items[2])
      assert_equal(1, state.numbers.items[3])
      ...
      decoder = Decoder:new(state)
      decoder:decode(snapshot)
      decoder:decode({ 255, 1, 128, 0, 0, 0, 3, 99, 128, 4, 4 })
      assert_equal(5, state.numbers:length())
      assert_equal(0, state.numbers.items[1])
      assert_equal(1, state.numbers.items[2])
      assert_equal(2, state.numbers.items[3])
      assert_equal(99, state.numbers.items[4])
      assert_equal(4, state.numbers.items[5])

      -- clear + unshift in the same tick: clear(); unshift(9); unshift(8)
      state = ArraySchemaInsertOps:new()
      decoder = Decoder:new(state)
      decoder:decode(snapshot)
      decoder:decode({ 255, 1, 10, 128, 0, 8, 128, 1, 9 })
      assert_equal(2, state.numbers:length())
      assert_equal(8, state.numbers.items[1])
      assert_equal(9, state.numbers.items[2])
    end)

    it("ArraySchemaUnshiftSchemaInstances", function()
      -- [Item(1), Item(2)] -> unshift(Item(0)); unshift(Item(-1))
      local state = ArraySchemaInsertOps:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 130, 3, 255, 2, 128, 0, 4, 128, 1, 5, 255, 4, 128, 1, 255, 5, 128, 2 })
      decoder:decode({ 255, 2, 128, 0, 7, 128, 1, 6, 255, 6, 128, 0, 255, 7, 128, 255 })

      assert_equal(4, state.items:length())
      assert_equal(-1, state.items.items[1].value)
      assert_equal(0, state.items.items[2].value)
      assert_equal(1, state.items.items[3].value)
      assert_equal(2, state.items.items[4].value)

      -- no refId leaks
      assert_equal(8, count_refs(decoder))
    end)

    it("ArraySchemaSortNoInsert", function()
      -- sort() emits REPLACE at occupied indexes (incl. the first) — must NOT insert
      local state = ArraySchemaInsertOps:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 130, 3, 255, 3, 128, 0, 4, 128, 1, 5, 128, 2, 6, 128, 3, 7, 128, 4, 8, 255, 4, 128, 163, 79, 110, 101, 129, 10, 130, 0, 255, 5, 128, 163, 84, 119, 111, 129, 30, 130, 1, 255, 6, 128, 165, 84, 104, 114, 101, 101, 129, 20, 130, 2, 255, 7, 128, 164, 70, 111, 117, 114, 129, 50, 130, 3, 255, 8, 128, 164, 70, 105, 118, 101, 129, 40, 130, 4 })
      assert_equal(5, state.players:length())
      assert_equal("One", state.players.items[1].name)
      assert_equal("Five", state.players.items[5].name)

      -- sort by y desc
      decoder:decode({ 255, 3, 0, 0, 8, 0, 1, 7, 0, 2, 6, 0, 3, 5, 0, 4, 4 })
      assert_equal(5, state.players:length())
      assert_equal("Five", state.players.items[1].name)
      assert_equal("Four", state.players.items[2].name)
      assert_equal("Three", state.players.items[3].name)
      assert_equal("Two", state.players.items[4].name)
      assert_equal("One", state.players.items[5].name)

      -- sort by x
      decoder:decode({ 255, 3, 0, 0, 4, 0, 1, 6, 0, 2, 5, 0, 3, 8, 0, 4, 7 })
      assert_equal(5, state.players:length())
      assert_equal("One", state.players.items[1].name)
      assert_equal("Three", state.players.items[2].name)
      assert_equal("Two", state.players.items[3].name)
      assert_equal("Five", state.players.items[4].name)
      assert_equal("Four", state.players.items[5].name)
    end)

    it("ArraySchemaStaleDeleteByRefId", function()
      ...
      -- silently: no write, no on_remove.
      local shared_patch = { 255, 2, 33, 4, 33, 5, 33, 6 }

      local old_state = ArraySchemaInsertOps:new()
      local old_decoder = Decoder:new(old_state)
      old_decoder:decode({ 128, 1, 129, 2, 130, 3, 255, 2, 128, 0, 4, 128, 1, 5, 128, 2, 6, 128, 3, 7, 128, 4, 8, 255, 4, 128, 0, 255, 5, 128, 1, 255, 6, 128, 2, 255, 7, 128, 3, 255, 8, 128, 4 })

      local fresh_state = ArraySchemaInsertOps:new()
      local fresh_decoder = Decoder:new(fresh_state)
      fresh_decoder:decode({ 128, 1, 129, 2, 130, 3, 255, 2, 128, 0, 6, 128, 1, 7, 128, 2, 8, 255, 6, 128, 2, 255, 7, 128, 3, 255, 8, 128, 4 })

      local old_remove_count = 0
      local fresh_remove_count = 0
      local old_callbacks = get_callbacks(old_decoder)
      local fresh_callbacks = get_callbacks(fresh_decoder)
      old_callbacks:on_remove("items", function(value, key) old_remove_count = old_remove_count + 1 end)
      fresh_callbacks:on_remove("items", function(value, key) fresh_remove_count = fresh_remove_count + 1 end)

      old_decoder:decode(shared_patch)
      fresh_decoder:decode(shared_patch)

      -- both converge to the server state: [3, 4]
      assert_equal(2, old_state.items:length())
      assert_equal(3, old_state.items.items[1].value)
      assert_equal(4, old_state.items.items[2].value)
      assert_equal(2, fresh_state.items:length())
      assert_equal(3, fresh_state.items.items[1].value)
      assert_equal(4, fresh_state.items.items[2].value)

      -- old client applied 3 deletes; fresh client skipped the 2 stale ones
      assert_equal(3, old_remove_count)
      assert_equal(1, fresh_remove_count)

      -- ref counts converge too (stale refIds were never tracked by fresh)
      assert_equal(6, count_refs(old_decoder))
      assert_equal(6, count_refs(fresh_decoder))
    end)

    it("ArraySchemaMove", function()
      ...
      local state = ArraySchemaInsertOps:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 130, 3, 255, 2, 128, 0, 4, 128, 1, 5, 128, 2, 6, 255, 4, 128, 1, 255, 5, 128, 2, 255, 6, 128, 3 })

      -- move(): swap arr[0] <-> arr[1] — carries MOVE (32) ops
      decoder:decode({ 255, 2, 32, 0, 5, 32, 1, 4, 255, 4, 128, 1 })
      assert_equal(3, state.items:length())
      assert_equal(2, state.items.items[1].value)
      assert_equal(1, state.items.items[2].value)
      assert_equal(3, state.items.items[3].value)

      -- move(): new Item(9) written at occupied slot 2 — MOVE_AND_ADD (160)
      decoder:decode({ 255, 2, 160, 2, 7, 255, 7, 128, 9 })
      assert_equal(3, state.items:length())
      assert_equal(2, state.items.items[1].value)
      assert_equal(1, state.items.items[2].value)
      assert_equal(9, state.items.items[3].value)
    end)

    --
    -- decode_resync fixtures below generated by schema-5.0
    ...
    it("ResyncGhostMap", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 128, 1, 162, 101, 50, 5, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4, 255, 5, 128, 163, 116, 119, 111, 129, 100, 130, 6 })

      local removed_items = {}
      local removed_keys = {}
      local callbacks = get_callbacks(decoder)
      callbacks:on_remove("units", function(u, k)
        table.insert(removed_items, u)
        table.insert(removed_keys, k)
      end)

      local ghost = state.units.items["e2"]

      -- offline: e2 died; its DELETE patch was never delivered
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4 })

      assert_equal(1, state.units:length())
      assert_not_equal(nil, state.units.items["e1"])
      assert_equal(1, #removed_items)
      assert_equal(ghost, removed_items[1]) -- the REAL previous instance
      assert_equal("e2", removed_keys[1])
      assert_equal(5, decoder.refs:count())
    end)

    it("ResyncPrunePrimitiveMap", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 2, 128, 0, 162, 116, 49, 10, 128, 1, 162, 116, 50, 20, 128, 2, 162, 116, 51, 30 })

      local removed = {}
      local callbacks = get_callbacks(decoder)
      callbacks:on_remove("trees", function(v, k) table.insert(removed, { v, k }) end)

      decoder:decode_resync({ 128, 1, 129, 2, 255, 2, 128, 0, 162, 116, 49, 10, 128, 2, 162, 116, 51, 30 })

      assert_equal(2, state.trees:length())
      assert_equal(10, state.trees.items["t1"])
      assert_equal(nil, state.trees.items["t2"])
      assert_equal(30, state.trees.items["t3"])
      assert_equal(1, #removed)
      assert_equal(20, removed[1][1])
      assert_equal("t2", removed[1][2])
    end)

    it("ResyncNestedCollection", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 164, 104, 101, 114, 111, 3, 255, 3, 128, 164, 104, 101, 114, 111, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 6, 128, 2, 7, 255, 5, 128, 0, 255, 6, 128, 1, 255, 7, 128, 2 })

      local hero_before = state.units.items["hero"]

      -- offline: hero lost the middle gem
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 0, 164, 104, 101, 114, 111, 3, 255, 3, 128, 164, 104, 101, 114, 111, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 7, 255, 5, 128, 0, 255, 7, 128, 2 })

      assert_equal(hero_before, state.units.items["hero"]) -- retained parent keeps identity
      assert_equal(2, state.units.items["hero"].gems:length())
      assert_equal(0, state.units.items["hero"].gems.items[1].price)
      assert_equal(2, state.units.items["hero"].gems.items[2].price)
    end)

    it("ResyncViewInterior", function()
      -- snapshots are view-encoded (ADD_BY_REFID ops)
      local state = ResyncArrayState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 255, 0, 128, 1, 255, 1, 129, 2, 2, 129, 4, 4, 129, 6, 6, 255, 2, 128, 161, 97, 129, 100, 130, 3, 255, 4, 128, 161, 98, 129, 100, 130, 5, 255, 6, 128, 161, 99, 129, 100, 130, 7 })
      assert_equal(3, state.arr:length())

      -- offline: b left the view — an interior index, not the tail
      decoder:decode_resync({ 255, 0, 128, 1, 255, 1, 129, 2, 2, 129, 6, 6, 255, 2, 128, 161, 97, 129, 100, 130, 3, 255, 6, 128, 161, 99, 129, 100, 130, 7 })

      assert_equal(2, state.arr:length())
      assert_equal("a", state.arr.items[1].name)
      assert_equal("c", state.arr.items[2].name)
    end)

    it("ResyncEmptiedCollection", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 128, 1, 162, 101, 50, 5, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4, 255, 5, 128, 163, 116, 119, 111, 129, 100, 130, 6 })
      ...
      local removals = 0
      local callbacks = get_callbacks(decoder)
      callbacks:on_remove("units", function() removals = removals + 1 end)

      decoder:decode_resync({ 128, 1, 129, 2 })

      assert_equal(0, state.units:length())
      assert_equal(2, removals)
    end)

    it("ResyncNoDoubleDecrement", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 164, 114, 105, 99, 104, 3, 128, 1, 164, 107, 101, 101, 112, 10, 255, 3, 128, 164, 114, 105, 99, 104, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 6, 128, 2, 7, 128, 3, 8, 128, 4, 9, 255, 5, 128, 0, 255, 6, 128, 1, 255, 7, 128, 2, 255, 8, 128, 3, 255, 9, 128, 4, 255, 10, 128, 164, 107, 101, 101, 112, 129, 100, 130, 11, 255, 11, 128, 0, 12, 128, 1, 13, 255, 12, 128, 0, 255, 13, 128, 1 })

      -- offline: 'rich' (5 nested gems) died — GC must release the subtree exactly once
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 1, 164, 107, 101, 101, 112, 10, 255, 10, 128, 164, 107, 101, 101, 112, 129, 100, 130, 11, 255, 11, 128, 0, 12, 128, 1, 13, 255, 12, 128, 0, 255, 13, 128, 1 })

      assert_equal(1, state.units:length())
      assert_equal(2, state.units.items["keep"].gems:length())
      assert_equal(7, decoder.refs:count())
    end)

    it("ResyncLateDelete", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 128, 1, 162, 101, 50, 5, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4, 255, 5, 128, 163, 116, 119, 111, 129, 100, 130, 6 })
      ...
      decoder:decode({ 255, 1, 64, 1 })

      assert_equal(1, removals)
      assert_equal(1, state.units:length())
    end)

    it("ResyncSurvivorIdentity", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 128, 1, 164, 103, 111, 110, 101, 5, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4, 255, 5, 128, 164, 103, 111, 110, 101, 129, 100, 130, 6 })

      local survivor = state.units.items["e1"]

      local adds = 0
      local hp_values = {}
      local callbacks = get_callbacks(decoder)
      callbacks:on_add("units", function() adds = adds + 1 end, false)
      callbacks:listen(survivor, "hp", function(hp) table.insert(hp_values, hp) end, false)

      -- offline: survivor's hp changed to 55, sibling died
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 255, 3, 128, 163, 111, 110, 101, 129, 55, 130, 4 })

      assert_equal(survivor, state.units.items["e1"]) -- same instance across resync
      assert_equal(0, adds) -- on_add must not re-fire for survivors
      assert_equal(1, #hp_values)
      assert_equal(55, hp_values[1])

      -- callbacks stay wired for live patches after the resync
      decoder:decode({ 255, 3, 129, 77 })
      assert_equal(2, #hp_values)
      assert_equal(77, hp_values[2])
    end)

    it("ResyncRekey", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 163, 111, 108, 100, 3, 255, 3, 128, 165, 109, 111, 118, 101, 114, 129, 100, 130, 4 })

      local instance = state.units.items["old"]

      local adds = {}
      local removes = {}
      local callbacks = get_callbacks(decoder)
      callbacks:on_add("units", function(_, k) table.insert(adds, k) end, false)
      callbacks:on_remove("units", function(_, k) table.insert(removes, k) end)

      -- offline: the same server instance moved to a new key
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 1, 163, 110, 101, 119, 3, 255, 3, 128, 165, 109, 111, 118, 101, 114, 129, 100, 130, 4 })

      assert_equal(instance, state.units.items["new"]) -- same instance under the new key
      assert_equal(nil, state.units.items["old"])
      assert_equal(1, #adds)
      assert_equal("new", adds[1])
      assert_equal(1, #removes)
      assert_equal("old", removes[1])
      assert_equal(5, decoder.refs:count())
    end)

    it("ResyncReplaceSameKey", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 161, 107, 3, 255, 3, 128, 165, 102, 105, 114, 115, 116, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 6, 255, 5, 128, 0, 255, 6, 128, 1 })

      local old_instance = state.units.items["k"]
      local old_refid = old_instance.__refid

      local adds = 0
      local removes = {}
      local callbacks = get_callbacks(decoder)
      callbacks:on_add("units", function() adds = adds + 1 end, false)
      callbacks:on_remove("units", function(u) table.insert(removes, u) end)

      -- offline: the entity at "k" died and a NEW one spawned at the same key
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 1, 161, 107, 7, 255, 7, 128, 166, 115, 101, 99, 111, 110, 100, 129, 100, 130, 8 })

      assert_equal("second", state.units.items["k"].name)
      assert_not_equal(old_instance, state.units.items["k"]) -- fresh instance
      assert_equal(1, #removes)
      assert_equal(old_instance, removes[1])
      assert_equal(1, adds)
      assert_equal(false, decoder.refs:has(old_refid)) -- old ref GC'd
      assert_equal(5, decoder.refs:count())
    end)

    it("ResyncDamaged", function()
      ...
      local state = ResyncStateV1:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 255, 1, 128, 0, 162, 112, 49, 2, 128, 1, 162, 112, 50, 3, 255, 2, 128, 1, 255, 3, 128, 2 })
      assert_equal(2, state.players:length())

      decoder:decode_resync({ 128, 1, 255, 1, 128, 0, 162, 112, 49, 2, 255, 2, 128, 1, 129, 162, 104, 105 })

      assert_equal(2, state.players:length()) -- ghost p2 retained, no crash
    end)

    it("ResyncTransient", function()
      local state = ResyncTransientState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 255, 1, 128, 0, 162, 101, 49, 3, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4 })
      -- transient entries arrive via patch only — never via full snapshots
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 255, 2, 128, 0, 162, 108, 49, 1, 128, 1, 162, 108, 50, 2, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4 })
      assert_equal(2, state.locals:length())

      -- the resync snapshot never mentions locals → presence rule leaves it alone
      decoder:decode_resync({ 128, 1, 255, 1, 128, 0, 162, 101, 49, 3, 128, 1, 162, 101, 50, 5, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4, 255, 5, 128, 163, 116, 119, 111, 129, 100, 130, 6 })

      assert_equal(2, state.locals:length()) -- transient data survives the sweep
      assert_equal(2, state.units:length())
    end)

    it("ResyncPlainAdditive", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 162, 101, 49, 3, 255, 3, 128, 163, 111, 110, 101, 129, 100, 130, 4 })
      decoder:decode({ 255, 1, 128, 1, 162, 101, 50, 5, 255, 5, 128, 163, 116, 119, 111, 129, 100, 130, 6 })
      -- (a DELETE patch for e1 was dropped — never delivered)
      decoder:decode({ 255, 5, 129, 42 })

      assert_equal(2, state.units:length()) -- plain decode stays additive

      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 1, 162, 101, 50, 5, 255, 5, 128, 163, 116, 119, 111, 129, 42, 130, 6 })

      assert_equal(1, state.units:length()) -- resync reconciles
      assert_equal(42, state.units.items["e2"].hp)
    end)

    it("ResyncTailTrim", function()
      local state = ResyncState:new()
      local decoder = Decoder:new(state)
      decoder:decode({ 128, 1, 129, 2, 255, 1, 128, 0, 164, 104, 101, 114, 111, 3, 255, 3, 128, 164, 104, 101, 114, 111, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 6, 128, 2, 7, 128, 3, 8, 255, 5, 128, 0, 255, 6, 128, 1, 255, 7, 128, 2, 255, 8, 128, 3 })

      -- offline: hero lost the last two gems
      decoder:decode_resync({ 128, 1, 129, 2, 255, 1, 128, 0, 164, 104, 101, 114, 111, 3, 255, 3, 128, 164, 104, 101, 114, 111, 129, 100, 130, 4, 255, 4, 128, 0, 5, 128, 1, 6, 255, 5, 128, 0, 255, 6, 128, 1 })

      assert_equal(2, state.units.items["hero"].gems:length())
      assert_equal(0, state.units.items["hero"].gems.items[1].price)
      assert_equal(1, state.units.items["hero"].gems.items[2].price)
      assert_equal(7, decoder.refs:count())
    end)
